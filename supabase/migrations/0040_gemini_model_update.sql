...
--   select vault.create_secret('gemini-x-flash', 'gemini_model', 'Gemini model for report routing');
create or replace function private.gemini_model() returns text
language sql stable security definer set search_path = '' as $$
  select coalesce((select decrypted_secret from vault.decrypted_secrets where name = 'gemini_model'), 'gemini-3.6-flash')
$$;
