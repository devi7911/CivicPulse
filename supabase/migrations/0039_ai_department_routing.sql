...
-- title or description changes, to stay well inside a free daily quota.

-- pg_net (used elsewhere in this app) is fire-and-poll-later: a queued request only becomes visible
-- to its background worker once the calling transaction commits, so a function cannot queue a
-- request and wait for its own answer in one call. The `http` extension makes a real blocking
-- call instead, so a single admin click gets a single, synchronous answer.
create extension if not exists http with schema extensions;

alter table public.issues
  add column if not exists ai_suggested_dept text,
  add column if not exists ai_suggested_at timestamptz,
  add column if not exists ai_suggested_hash text;

grant select (ai_suggested_dept, ai_suggested_at, ai_suggested_hash) on public.issues to authenticated;

-- Reads the key from Vault, exactly like private.contact_key(). Run once, from the SQL editor,
-- before this feature works:
--   select vault.create_secret('<your key>', 'gemini_api_key', 'Google Gemini API key for report routing');
create or replace function private.gemini_key() returns text
...
begin
  ...
  -- Cached: the report has not changed since it was last suggested.
  ...
  -- A generous but finite daily ceiling, so a bug here cannot silently exhaust the free quota.
  ...
  -- Only the parsing of the model's own reply is allowed to fail softly; validation errors above
  -- (no permission, bad input, quota) still raise normally so the admin sees what actually went wrong.
  begin
