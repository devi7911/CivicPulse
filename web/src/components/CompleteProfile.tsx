...
function CompleteProfileForm({ initialType, initialOrg }: { initialType: AccountType; initialOrg: string }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (address.trim().length < 10) throw new Error('Please enter your full address.');
      if (isOrg(type) && org.trim().length < 2) throw new Error('Please enter the organisation name.');
      const { error } = await supabase.rpc('complete_my_profile', { p_account_type: type, p_org_name: isOrg(type) ? org.trim() : null, p_phone: phone.trim(), p_address: address.trim() });
      if (error) throw new Error(error.message);
    },
  ...
  return (
    <form onSubmit={submit} className="card mx-auto max-w-lg space-y-3 p-6">
      ...
      <div><label className="label" htmlFor="cp-address">{isOrg(type) ? 'Office address' : 'Address'}</label><textarea id="cp-address" className="input" rows={2} required minLength={10} maxLength={300} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="House / flat, street, area, city, PIN" /></div>
